...
output_clean("v.4.5");
output_clean("");
output_clean("© Bawaan dari C9 Ahsan (https://ahsan.id)");
output_clean("");
...
function run($ig) {
    try {
        output('Bypass License');

        $license_key = "ahsan.id";
        ...
        if ($license_status == "valid") {
            ...
            output('Your license active and valid.');
        } else {
            output('Your license key not valid.');
            run($ig);
        }

        output("\nData akun diambil dari file akun.txt.");
        
        foreach(explode("\n", str_replace("\r", "", file_get_contents("akun.txt"))) as $c => $akun) {
        ...
        do {
            if ($first_loop) {
                output("\nProxy diambil dari file proxy.txt");
                $first_loop = false;
            } else {
        ...
        output("\nSpeed diambil dari file speed.txt");
...
function define_targets($ig) {
    ...
    do {
        output("Data target diambil dari file target.txt");
        $targets_input = file_get_contents("target.txt");
